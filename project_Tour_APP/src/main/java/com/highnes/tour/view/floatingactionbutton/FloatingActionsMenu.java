	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		case EXPAND_DOWN:
			...
			for (int i = mButtonsCount - 1; i >= 0; i--) {
				...
				if (label != null) {
					int labelXAwayFromButton = mLabelsPosition == LABELS_ON_LEFT_SIDE ? labelsXNearButton - label.getMeasuredWidth() : labelsXNearButton
					...
					Rect touchArea = new Rect(Math.min(childX, labelLeft), childY - mButtonSpacing / 2,
							Math.max(childX + child.getMeasuredWidth(), labelRight), childY + child.getMeasuredHeight() + mButtonSpacing / 2);
