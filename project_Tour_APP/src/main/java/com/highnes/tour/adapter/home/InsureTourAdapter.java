 ...
 * </PRE>
 */
public class InsureTourAdapter extends BaseListAdapter<TourSafestInfo.SafestInfo> {

	private int index = 0;

	public InsureTourAdapter(Context paramContext) {
		super(paramContext);
		initOptions();
	}
	...
	public View getView(final int position, View view, ViewGroup viewGroup) {
		...
		TourSafestInfo.SafestInfo info = (TourSafestInfo.SafestInfo) getItem(position);
		// 设置页面显示值
	...
	private class MyOnClickListener implements OnClickListener {
		TourSafestInfo.SafestInfo info;
		int type;
		int position;

		public MyOnClickListener(int position, TourSafestInfo.SafestInfo info, int type) {
			this.info = info;
			this.type = type;
	...
	public interface OnChecked {
		 ...
		 *            类别
		 */
		void onChecked(int position, TourSafestInfo.SafestInfo info, int type);
	}
