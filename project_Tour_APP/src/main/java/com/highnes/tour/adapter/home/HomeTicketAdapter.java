 ...
 * </PRE>
 */
public class HomeTicketAdapter extends BaseListAdapter<HomeAllInfo.SoldInfo> {

	public HomeTicketAdapter(Context paramContext) {
		super(paramContext);
		initOptions();
	}
	...
	public View getView(final int position, View view, ViewGroup viewGroup) {
		...
		HomeAllInfo.SoldInfo info = (HomeAllInfo.SoldInfo) getItem(position);
		// 设置页面显示值
		holder.tvTitle.setText(info.SoldName);
		holder.tvLoc.setText(info.SCity);
		holder.tvMoneyNow.setText(info.SNewPrice + "");
//		holder.tvMoneyNow.setText(ArithUtil.format2Int(info.SNewPrice) + "");
	...
	private class MyOnClickListener implements OnClickListener {
		private HomeAllInfo.SoldInfo mInfo;

		public MyOnClickListener(HomeAllInfo.SoldInfo mInfo) {
			this.mInfo = mInfo;
		}
	...
	public interface OnCellItem {
		void onItem(HomeAllInfo.SoldInfo info);
