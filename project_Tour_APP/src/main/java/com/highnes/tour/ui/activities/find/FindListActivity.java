...
import com.highnes.tour.adapter.find.FindListAdapter;
 ...
 * </PRE>
 */
public class FindListActivity extends BaseTitleActivity implements OnRefreshListener {
	...
	protected void initView2T(Bundle savedInstanceState) {
		...
		mTitle = getIntent().getExtras().getString("mTitle", "发现");
		setTitle(mTitle);
		showBackwardView("", SHOW_ICON_DEFAULT);
		adapter = new FindListAdapter(mContext);
	...
	protected void setListener2T() {
		...
		lvList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				FindListInfo.NotesInfo info = (FindListInfo.NotesInfo) lvList.getAdapter().getItem(position);
	...
	private void requestByData() {
		...
		paramDatas.put("TypeID", mTypeID);
		showLoading();
		new NETConnection(mContext, UrlSettings.URL_FIND_LIST, new NETConnection.SuccessCallback() {
