...
import com.highnes.tour.adapter.home.HomeTimesDetailAdapter;
import com.highnes.tour.adapter.home.TimeCommentAdapter;
import com.highnes.tour.beans.home.HomeTimeDetailInfo;
import com.highnes.tour.beans.home.HomeTimeDetailInfo.SoldInfo;
 ...
 * </PRE>
 */
public class TimeDetailActivity extends BaseActivity implements OnRefreshListener {
	/** 改变第1个菜单切换的背景颜色 */
	...
	private String mID;
	// 当前门票的信息
	private HomeTimeDetailInfo mInfo;

	@Override
	@LayoutRes
	protected int getLayoutId() {
		return R.layout.activity_home_time_detail;
	}
	...
	protected void initView(Bundle savedInstanceState) {
		...
		initWebView();
		commentAdapter = new TimeCommentAdapter(mContext);
	...
	public void onClick(View v) {
		...
		case R.id.ll_comment_more: {
			...
			bundle.putString("mNewPrice", mNewPrice);
			openActivity(TimeCommentListActivity.class, bundle);
		}
			break;
		default:
			break;
	...
	private void shareApp() {
		ActionSheetDialog asd = new ActionSheetDialog(TimeDetailActivity.this).builder();
		...
		asd.setmOnItemClick(new ActionSheetDialog.OnItemClick() {
			...
			public void onClick(int rsid) {
				...
				case R.id.ll_share_qq:
					...
					ShareHelper.share(2, TimeDetailActivity.this, getLoading(), String.format(UrlSettings.URL_SHARE_TICKET, mTicketID,userID), getResources()
	 ...
	 * @param info
	 */
	private void initAreaInfo(HomeTimeDetailInfo info) {
		try {
			...
			initTicketBuy(info.sold, info.SoldCount);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @param info
	 */
	private void initTicketBuy(List<HomeTimeDetailInfo.SoldInfo> info, String SoldCount) {
		try {
			// TODO 数量
			adapter = new HomeTimesDetailAdapter(mContext, SoldCount);
			lvBuy.setAdapter(adapter);
			adapter.setList(info);
			adapter.setCallback(new HomeTimesDetailAdapter.Callback() {

				@Override
				public void onSuccess(SoldInfo info, int type) {
					if (0 == type) {
						// 票型说明
						Bundle bundle = new Bundle();
						...
						if (AppUtils.isLogin(mContext)) {
							...
							bundle.putString("mNewPrice", mNewPrice);
							bundle.putString("mOverTime", mOverTime);
	 ...
	 * 显示Banner
	 */
	private void initBanner(final List<HomeTimeDetailInfo.ImageUrlsInfo> info) {
		try {
	...
	private void loadWeb() {
		if (isFristLoadWeb) {
			web.loadUrl(UrlSettings.URL_H5_HOME_TICKET_DETAIL + mInfo.AreaID);
	...
	private void checkWeiXin(boolean isWeiXinCricle) {
		...
		if (api.isWXAppInstalled()) {
			...
			if (isWeiXinCricle) {
				String userID = SPUtils.get(mContext, PreSettings.USER_ID.getId(), PreSettings.USER_ID.getDefaultValue()).toString();
				ShareHelper.share(0, TimeDetailActivity.this, getLoading(), String.format(UrlSettings.URL_SHARE_TICKET, mTicketID,userID),
						getResources().getString(R.string.share_title), getResources().getString(R.string.share_text), urlIamge, new MyUMShareListener());
			} else {
				String userID = SPUtils.get(mContext, PreSettings.USER_ID.getId(), PreSettings.USER_ID.getDefaultValue()).toString();
				ShareHelper.share(1, TimeDetailActivity.this, getLoading(), String.format(UrlSettings.URL_SHARE_TICKET, mTicketID,userID),
						getResources().getString(R.string.share_title), getResources().getString(R.string.share_text), urlIamge, new MyUMShareListener());
			}
		} else {
			showToast("还没有安装微信");
