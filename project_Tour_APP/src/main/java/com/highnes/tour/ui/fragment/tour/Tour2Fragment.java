...
import com.highnes.tour.adapter.home.tour.Tour2HotAdapter;
 ...
 * </PRE>
 */
public class Tour2Fragment extends BaseFragment implements OnRefreshListener {
	/** 广播回调监听_刷新 */
	public static final String ACTION_CALLBACK_TOUR2 = "com.highnes.tour.action_callback_tour2";
	...
	// 热门城市
	private Tour2HotAdapter hotAdapter;
	...
	private void registBoradcast() {
		...
		intentFilter.addAction(ACTION_CALLBACK_TOUR2);
	...
	protected @LayoutRes
	int getLayoutId() {
		return R.layout.fragment_home_tour2;
	}
	...
	protected void initView(View view, Bundle savedInstanceState) {
		...
		hotAdapter = new Tour2HotAdapter(mContext);
	...
	protected void setListener() {
		...
		gvTop.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				bundle.putString("nearbyName", "国内游");
		...
		gvHot.setOnItemClickListener(new OnItemClickListener() {
			...
			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				Tour2Info.HotCityInfo info = (Tour2Info.HotCityInfo) gvHot.getAdapter().getItem(position);
				Bundle bundle = new Bundle();
				bundle.putString("nearbyType", "hot");
				bundle.putString("nearbyName", "国内游");
				bundle.putString("nearbyValue", info.CityName);
				openActivity(FilterTour2Activity.class, bundle);
			}
		});
	...
	private void requestByData() {
		...
		showLoading();
		new NETConnection(mContext, UrlSettings.URL_HOME_TOUR_2, new NETConnection.SuccessCallback() {
	...
	private void parseJsonData(String result) {
		if (StringUtils.isEquals("1", JsonUtils.getString(result, "status", "0"))) {
			...
			if ((info.HotArea == null || info.HotArea.size() == 0) && (info.HotCity == null || info.HotCity.size() == 0) && (info.HotTourism == null
					|| info.HotTourism.size() == 0)) {
				llShowData.setVisibility(View.VISIBLE);
			} else {
	 ...
	 * @param info
	 */
	private void initCity(List<Tour2Info.HotCityInfo> info) {
		try {
			if (info != null && info.size() > 0) {
